        ...
        if(totalPages > 0)
        {
            pages.push(
                <li className={`page-item ${(data.current_page == 1) ? 'disabled' : ''}`} key={`first ${data.current_page}`}>
                    <button className="page-link" onClick={() => setCurrentPage(data.current_page - 1)}>Previous</button>
                </li>
            )
            for (let i = 1; i <= totalPages; i++) {
                ...
                pages.push(
                    <li className="page-item" key={i}>
                        <button className={`page-link ${currentActiveClass ? 'active' : ''} `} onClick={() => setCurrentPage(i)}>{i}</button>
                    </li>
                )
            }
            pages.push(
                <li className={`page-item ${(data.current_page == data.last_page) ? 'disabled' : ''}`}  key={`last ${data.last_page}`} >
                    <button className="page-link" onClick={() => setCurrentPage(data.current_page + 1)}>Next</button>
                </li>
            )
        }
        return pages;
